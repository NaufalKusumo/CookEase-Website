...
<body class="bg-gray-50">
    ...
    <main class="container mx-auto p-8 mt-24">
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-gray-800">Selamat datang kembali, {{ $user->name }}!</h1>
            <p class="text-lg text-gray-600">Berikut resep dan tips dapur yang telah kamu buat.</p>
        </div>

        <!-- My Recipes Section -->
        <section class="mb-12">
            <h2 class="text-2xl font-semibold mb-4">Resepku</h2>
            @if ($recipes->isEmpty())
                <p>Kamu belum membagikan resep. <a href="{{ route('recipes.create') }}" class="text-green-600 font-bold hover:underline">Buat resep sekarang!</a></p>
            @else
        ...
        <!-- My Tips Section -->
        <section>
            <h2 class="text-2xl font-semibold mb-4">Tipsku</h2>
             @if ($tips->isEmpty())
                <p>Kamu belum membagikan tips dapur. <a href="{{ route('tips.create') }}" class="text-green-600 font-bold hover:underline">Buat tips dapur sekarang!</a></p>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($tips as $tip)
