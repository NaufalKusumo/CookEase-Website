...
<body class="bg-gray-50">
    ...
    <main class="container mx-auto p-8">
        ...
        <form id="recipe-form" method="POST" action="{{ route('recipes.store') }}" enctype="multipart/form-data">
            ...
            <!-- Title -->
            <input type="text" name="title" placeholder="Judul Resep (Wajib)" value="{{ old('title', $recipe->title) }}" class="w-full text-4xl font-bold bg-gray-100 p-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
            @error('title') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            <p class="mt-2 text-gray-600">Author: {{ Auth::user()->name }}</p>

            <!-- Description -->
            <textarea name="description" placeholder="Keterangan singkat dibalik resepmu (Optional)" class="mt-4 w-full text-lg bg-gray-100 p-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('description', $recipe->description) }}</textarea>
...
300 g ketan
Garam..." class="mt-4 w-full text-lg bg-gray-100 p-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('ingredients', $recipe->ingredients) }}</textarea>
...
2. Tiriskan air..." class="mt-4 w-full text-lg bg-gray-100 p-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('instructions', $recipe->ingredients) }}</textarea>
