...
<body class="bg-gray-50">
    ...
    <main class="container mx-auto p-8 max-w-4xl">
        <form id="tip-form" method="POST" action="{{ route('tips.store') }}" enctype="multipart/form-data">
            @csrf <!-- IMPORTANT: Laravel security token -->

            <!-- Title -->
            <input type="text" name="title" placeholder="Judul Tip (Wajib)" value="{{ old('title') }}" class="w-full text-4xl font-bold bg-gray-100 p-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="3" placeholder="Tuliskan ringkasan singkat tipmu di sini. Ini akan muncul di kartu homepage." class="mt-2 w-full text-lg bg-gray-100 p-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('description') }}</textarea>
